...
public interface RatingRepository extends JpaRepository<Rating, Long> {
    ...
    @Transactional
    @Query(value = "SELECT r FROM Ratings r JOIN r.subject.teachers t WHERE r.task.id = ?1 AND t.id = ?2")
    List<Rating> findByTaskIdAndTeacherId(long taskId, long teacherId);

    @Transactional
    @Query(value = "SELECT r FROM Ratings r JOIN r.subject.teachers t WHERE r.id = ?1 AND t.id = ?2")
