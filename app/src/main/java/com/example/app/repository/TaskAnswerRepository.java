...
public interface TaskAnswerRepository extends JpaRepository<TaskAnswer, Long> {
    @Transactional
    @Query(value = "SELECT a FROM TaskAnswer a WHERE a.student.id = ?1 AND a.task.id = ?2")
    Optional<TaskAnswer> findByStudentIdAndByTaskId(long studentId, long taskId);

    @Transactional
    @Query(value = "SELECT a FROM TaskAnswer a WHERE a.task.teacher.id = ?1 AND a.task.id = ?2")
